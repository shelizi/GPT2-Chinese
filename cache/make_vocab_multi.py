...
def main():
    ...
    files = os.listdir(args.raw_data_path)
    lines=[]
    for i, raw_file_name in enumerate(tqdm(files)):
        full_path = os.path.join(args.raw_data_path, raw_file_name)
        with open(full_path) as file:
            lines.append(lac.cut(file.read(), text=True))
